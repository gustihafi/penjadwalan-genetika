...
class Akses extends CI_Controller{
	...
	function ActLogin(){
        ...
        $cek_login_guru = $this->m_guru->GetGuru("nip = '$username' and password = '$md5_securepass'")->num_rows();
        $tampil_guru = $this->m_guru->GetGuru("nip = '$username' and password = '$md5_securepass'")->row_array();
        ...
        if ($cek_login_aktif == 1) {
            ...
            redirect('web');
        } else if ($cek_login_guru == 1){
            $this->session->set_userdata('ses_id',$tampil_guru['kode']);
            $this->session->set_userdata('ses_id_guru',$tampil_guru['kode']);
            $this->session->set_userdata('ses_nip',$tampil_guru['nip']);
            $this->session->set_userdata('ses_nama', $tampil_guru['nama']);
            $this->session->set_userdata('ses_alamat',$tampil_guru['alamat']);
            $this->session->set_userdata('ses_telp', $tampil_guru['telp']);
            $this->session->set_userdata('ses_level', 'guru');
            redirect('web');
        }else{
    ...
    function LogApp(){
        ...
        $this->session->unset_userdata('ses_id_guru');
