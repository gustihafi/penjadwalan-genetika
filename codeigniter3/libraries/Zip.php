...
class CI_Zip  {
	...
	function read_file($path, $preserve_filepath = FALSE){
		...
		if (FALSE !== ($data = file_get_contents($path))){
			...
			if ($preserve_filepath === FALSE){$name = preg_replace("|.*/(.+)|", "\\1", $name);}
	...
	function read_dir($path, $preserve_filepath = TRUE, $root_path = NULL){
		...
		if ($root_path === NULL){$root_path = dirname($path).'/';}
		while (FALSE !== ($file = readdir($fp))){
			...
			if (@is_dir($path.$file)){
				...
				if (FALSE !== ($data = file_get_contents($path.$file))){
					$name = str_replace("\\", "/", $path);
					if ($preserve_filepath === FALSE){$name = str_replace($root_path, '', $name);}
	...
	function archive($filepath){
		if ( ! ($fp = @fopen($filepath, FOPEN_WRITE_CREATE_DESTRUCTIVE))){return FALSE;}
		flock($fp, LOCK_EX);
	...
	function download($filename = 'backup.zip'){
		if ( ! preg_match("|.+?\.zip$|", $filename)){$filename .= '.zip';}
		$CI =& get_instance();
